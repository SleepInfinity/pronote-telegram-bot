...
async def make_languages_keyboard():
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(text=f"{language['name']} {language['flag']}", callback_data=f"set_lang_{language['code']}")
            ]
...
async def setup_user_lang(user_id):
    ...
    await bot.send_message(user_id, languages["en"]["choose_language"], reply_markup=await make_languages_keyboard())
    return False
